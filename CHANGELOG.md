...
- **Chat was silently using the rate-limited API path instead of your Max plan.** The browser "mode pill" labeled CLI mode as *"browser tools off"* and Direct mode as *"Claude can drive"* — but that's backwards: **CLI mode is a `claude -p` session** with your **Max subscription + all MCP servers (incl. browser control via ccm-browser's 202 tools) + `--resume` continuity**, while **Direct mode** is the raw Anthropic **API key** path (tier rate limits, **no MCP servers**, only a few hand-wired browser tools). The misleading labels lured users into Direct mode → frequent 429s (confirmed: the rate-limit errors came from the SDK/Direct path). Relabeled the pill, toast, and model-menu Mode section to make **CLI the recommended default** and Direct an explicit API-key fallback. `directMode` isn't persisted, so a reload returns the chat to CLI mode.

- **Expose all MCP servers to the chat.** Claude Code 2.x loads MCP servers from `~/.claude.json`, not `~/.claude/settings.json` — so servers configured only in settings.json (filesystem, memory-engine, pinpoint) never loaded in the chat/CLI (verified: `claude mcp list` showed 31 servers but none of those three). On startup CCM now mirrors any settings.json-only MCP server into `~/.claude.json`'s root `mcpServers` so the chat exposes them too. **Guarded + non-destructive**: reads `~/.claude.json` directly and requires a clean JSON parse before writing (never clobbers the user's config on a read error); only adds missing entries, writes only when something changed.

- **MCP panel only showed some servers — now lists them all.** The in-app MCP Servers panel read only `~/.claude/settings.json` (legacy), so it showed 4 servers while the user's other ~8 (lovable, ideogram, netlify, memorify, elevenlabs, midjourney, …) live in **`~/.claude.json`** — Claude Code 2.x's primary config, where `/mcp` actually reads. `mcp:list` now merges all three sources (**user** `~/.claude.json` · **global** `~/.claude/settings.json` · **project** `.claude/settings.json`), deduped by name (user > global > project) and each tagged with the file it lives in. Add/edit/remove are now scope-aware via that mapping (a new `user` scope routes to `~/.claude.json`), so editing or removing a server targets the correct file; new servers default to `~/.claude.json` so the CLI picks them up. The panel gains a `user` scope badge + radio. (Requires full restart for the main-process handler.)

- **Chat rate-limit handling — stale-retry bug + no escape hatch.** On a 429 the chat shows a countdown then auto-retries once, but (1) the countdown timer was only cancelled on a *new* 429, so sending a fresh message during the wait could let the stale timer fire a **duplicate retry** on top of it; and (2) during the wait (up to 120s when the API sends no `retry-after`) you were stuck. Now a fresh send cancels any pending countdown, and the countdown shows **Retry now** / **Cancel** controls (Cancel drops the message back into the composer so it isn't lost). The per-tick update only rewrites the seconds, so the buttons aren't re-bound under your click.
- **Workspace restore threw "root must be of type branch" (Phase 23 regression).** The terminal-strip could prune a saved layout down to a leaf/empty root, but dockview requires a **branch** root — so `fromJSON` threw on restore and the layout fell back to default (i.e., was lost). The strip now nulls the grid when nothing restorable survives, and `saveCurrentLayout` persists `null` in that case, so restore cleanly builds the default with no throw. Self-heals on the next save.
...
- **Director drive channel — the Director now wakes its agents (Phase 26f)** — *the missing half of the loop, surfaced by a live test.* The Director could push tasks to the board but had no way to make the agent terminals **read** them, so tasks sat in "doing" forever. Now:
  ...
  - **`agent_send` MCP tool** (→ 197 tools): inject an ad-hoc prompt into any role's terminal — for kickoffs, nudges, or re-prodding a stalled agent.
  - **`director_status` reports `liveTerminals`** — which roles currently have a reachable terminal.
  - Kickoff text avoids leading `/` and `@` (they trigger the CLI's slash-command / file-completion and would corrupt the prompt). Writes go straight to the PTY via `ptyProc.write(text + "\r")` — no shell, no quoting. Verified with a 10-assertion drive-channel test (routing, nudged/unreachable split, correct PTY targeting, submission, exit cleanup).
